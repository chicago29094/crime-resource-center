...
export default function ArrestsSearchForm(props) {
    ...
    function handleSearchChange(event) {
		...
		const searchRequest = {
            ...
            detailedOffense: formFields.detailedOffense,
    ...
    return (
        <div className="arrest-search-form">
            <h2>Arrests Search Form</h2>

            <Form id="arrestSearchForm" onSubmit={handleSearchSubmit}>
                ...
                <Form.Group controlId="detailedOffense">
                    <Form.Label>Detailed Offense Type:</Form.Label>
                    <Form.Control as="select" required onChange={handleSearchChange}  value={formFields.detailedOffense.value} defaultValue={formFields.stateSearch.value}>
                    <option value="" >Select...</option>
                    {
                        Object.entries(fbiControllers["arrest-data"]["detailedOffense"]).map(
                            (key, index) => {
